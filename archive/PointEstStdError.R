...
mergeback <- 1 # flag for merging the new data back to the original one after calculating the stats. used at the end
{
  dist1 <- 0.25
  dist2 <- 0.5
  dist3 <- 1
...
##path
transactions <- read.csv("production/intermediate_output/Flint_Analysis_3.csv")
...

{
  ...
  trans_addr <-
    trans_addr[!is.na(trans_addr$PropertyAddressLatitude), ]        # filter out the data that don't have valid longitude/latitude
  trans_addr <-
    trans_addr[!is.na(trans_addr$PropertyAddressLongitude), ]
  ...
  coordinates(trans_coords) <-
    c("PropertyAddressLongitude", "PropertyAddressLatitude")   # coordinates for all the housing points
...
for (dist in dists)
{
  ...
  o_transactions[, cumulative.prob] <- NA
  standard_error <- paste(name2_base, dist_s, sep = "_")
  dev <- paste(standard_error, "km", sep = "")
  transactions[, dev] <- NA
  o_transactions[, dev] <- NA
  
  ###########
  #o_transactions[,cumulative.prob] = 0
  #o_transactions[,dev] = 0
  # }
  ###########
  ...
  for (j in 1:nrow(transactions))
  {
    ...
    valid_tests <-
      lead_tests[as.Date(lead_tests$Results_Se, format = "%m/%d/%Y") < as.Date(transactions[j, ]$RecordingDate), ]
    temp_tests_coords <- valid_tests
    temp_tests_coords <-
      temp_tests_coords[!is.na(temp_tests_coords$X), ]
    temp_tests_coords <-
      temp_tests_coords[!is.na(temp_tests_coords$Y), ]
    coordinates(temp_tests_coords) <- c("X", "Y")
    distVec <-
      spDistsN1(temp_tests_coords, trans_coords[j, ], longlat = TRUE)
      ...
      # there exists lead tests happened before the transactions
    {
      ...
      log$positive.tests_byDay <-
        ave(log$Lead_mgl, log$Date, FUN = sum)
      log$tests_byDay <- ave(log$test, log$Date, FUN = sum)
      
      # subset of results by day
      daily <- log[!duplicated(log$Date), ]
      daily <-
        daily[as.Date(daily$Date) < as.Date(transactions[j, ]$RecordingDate),]
      ...
      for (i in 1:n) {
        ...
        cumul_tests <-
          sum(daily[which(daily$Date <= my.date), "tests_byDay"])
        cumul_positive.tests <-
          sum(daily[which(daily$Date <= my.date), "positive.tests_byDay"])
        
        daily[i, "cumulative.prob"] <-
          cumul_positive.tests / cumul_tests
      }
      ...
      for (i in 1:n)
      {
        ...
        cumul_tests <-
          sum(daily[which(daily$Date <= my.date), "tests_byDay"])
        cumul_deviation <-
          sum(daily[which(daily$Date <= my.date), "deviation_byDay"])
        #!!!!!
        daily[i, "cumulative.standard.deviation"] <- # -> cumulative standard error
          (cumul_deviation / cumul_tests) / sqrt(cumul_tests)
      }
...
write.csv(o_transactions, "production/intermediate_output/Flint_Analysis_4.csv")
}
